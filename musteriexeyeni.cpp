...
void parcalasiparisler(char okunanparca[]) {
    ...
    int i = 0;
    char* key;
    key = strtok(okunanparca, ","); //strtok fonksiyonunu kullanip okunan parcayi tanitiriz, ilk bolum alinir yani siparisid
    key = strtok(NULL, ","); //tekrar strok kullanilir ikinci parca yani kisiid ele alinir
...
void parcalayemeklistesi(char okunanparca[]) {
    ...
    if (strcmp(key, "0")) { //eger key 1'e eþitse yani yemek mevcutsa, parcayi okumaya baslar
        while (key != NULL) {
            switch (i)
            {
            case 0: //mevcutluk
...
baslangic:
    ...
    case 1: //yemek listesi goruntuleme ve siparis verme
        ...
        if (key == 1) {
            ...
            fprintf(dosya, "%d,%s,2,%s,%04d,%04d,%d,%s*", siparisidolustur(kisiid),kisiid, yemekadlari[kod], guncelsaat(), dakikatopla(guncelsaat(),atoi(yemeksureleri[kod])), tarih, yemekfiyatlari[kod]);
            fclose(dosya);
